  ...
  const options: ChartOptions<"line"> = {
    ...
    plugins: {
      ...
      tooltip: {
        ...
        callbacks: {
          title: (tooltipItems) => {
            // 1. Safety check for isMounted and ensuring the parsed X value exists
            const xValue = tooltipItems[0]?.parsed?.x;
            if (!isMounted || xValue === null || xValue === undefined)
              return "";
          ...
          label: (context) => {
            // 2. Safety check for the parsed Y value
            const yValue = context.parsed.y;
            const formattedY = yValue !== null ? yValue.toFixed(2) : "0.00";

            return `${label}: ${formattedY}${unit}`;
          },
        },
      },
    },
    ...
    <div className="bg-white rounded-xl shadow-lg p-3 sm:p-6 hover:shadow-xl transition-shadow duration-300">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-0 mb-4">
        ...
        <div className="flex items-center justify-between sm:justify-start sm:space-x-4">
          ...
          <div className="text-xs sm:text-sm text-gray-400">{timeString}</div>
        </div>
      </div>
